...
echo "${emailbodytextH}" | mail -a ${filenameFLH} -s "report from Argo netCDF-to-BUFR processing on exvmarproc01" ${emailToList} 

# 3a. COUNTS remove hourly file and start a new, empty file
cp ${filenameH} ${filenameHO}
rm ${filenameH}
touch ${filenameH} 

# 3b. FILE LISTS remove hourly file and start a new, empty file
